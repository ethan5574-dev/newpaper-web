...
export default function Home() {
  ...
  return (
    <div className="bg-white min-h-screen pt-16" onClick={handleClickOutside}>
          ...
          <div className="mb-6 flex justify-between items-center">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-50 cursor-pointer"
              >
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-48 overflow-hidden rounded-md border border-zinc-200 bg-white shadow-lg z-10">
                  <button
                    ...
                    className={`flex w-full items-center px-4 py-2 text-sm hover:bg-zinc-50 cursor-pointer ${
                      filterType === 'all' ? 'bg-zinc-100 text-zinc-900' : 'text-zinc-700'
                    }`}
                  >
                    All Posts
                  </button>
                  <button
                    ...
                    className={`flex w-full items-center px-4 py-2 text-sm hover:bg-zinc-50 cursor-pointer ${
                      filterType === 'top30' ? 'bg-zinc-100 text-zinc-900' : 'text-zinc-700'
                    }`}
                  >
                    Top 3 (30 days)
                  </button>
            ...
            {filterType === 'all' ? (
              ...
              <article key={post.post_id} className="group mb-6 break-inside-avoid overflow-hidden rounded-md bg-white shadow">
                ...
                <div className="p-4">
                  ...
                  <h3 className="mt-1 text-base font-semibold text-zinc-900 cursor-pointer hover:text-sky-700" onClick={() => router.push(`/post/${post.post_id}`)}>{post.title}</h3>
                  <p className="mt-2 line-clamp-3 text-sm text-gray-600">{stripHtml(post.content).slice(0, 160)}</p>
                  <div className="mt-3 flex items-center justify-between">
                    <span className="text-xs font-semibold text-sky-700">{post.author.username}</span>
